...
Widget getRoomListTile(DashboardModel model, Room room, bool selected, bool hasNewMessage, bool isArchived, List<Message> messages, List<ReservationSlotItem> slots, {required Function(Room) didSelectRoom}) {
  ...
  room.users.removeWhere((element) => element.id == FirebaseChatCore.instance.firebaseUser?.uid);
  ...
  return TextButton(
    style: ButtonStyle(
        backgroundColor: MaterialStateProperty.resolveWith<Color>(
            ...
            return selected ? model.paletteColor : Colors.transparent; // Use the component's default.
          },
        ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Reservation with ${room.name ?? 'a New Listing'}', style: TextStyle(color: selected ? model.webBackgroundColor : (hasNewMessage) ? model.paletteColor : model.disabledTextColor, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis),
                    const SizedBox(height: 3),
                    Text(textMessage?.text ?? 'Start Chatting', style: TextStyle(color: selected ? model.webBackgroundColor : (hasNewMessage) ? model.paletteColor : model.disabledTextColor, fontWeight: hasNewMessage ? FontWeight.bold : FontWeight.normal), maxLines: 2, overflow: TextOverflow.ellipsis),
                      ...
                      children: [
                        ...
                        if (!isArchived && slots.isNotEmpty && resHasStarted && getAllRemainingDates(slots).isNotEmpty) Expanded(
                          child: Container(
                              ...
                              child: Padding(
                                padding: const EdgeInsets.all(6.0),
                                child: Text('Coming Up: ${DateFormat.yMMMd().format(getAllRemainingDates(slots).first)} at ${DateFormat.jm().format(getAllRemainingDates(slots).first)}', style: TextStyle(color: selected ? model.webBackgroundColor : model.disabledTextColor, fontWeight: hasNewMessage ? FontWeight.bold : FontWeight.normal), overflow: TextOverflow.ellipsis,),
                              )
                          ),
                        ),
                        ...
                        if (isArchived) Container(
                            ...
                            child: Padding(
                              padding: const EdgeInsets.all(6.0),
                              child: Text('Finished', style: TextStyle(color: selected ? model.webBackgroundColor : model.paletteColor, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis),
                            )
                        ),
                      ],
                    )
